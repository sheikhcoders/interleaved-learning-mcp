...
// ============================================
// A research-backed cognitive learning tool that helps
// implement interleaved practice for optimal retention
// ============================================
...
// Interleaving patterns with descriptions
const INTERLEAVING_PATTERNS: Record<string, string> = {
...
// Generate interleaved sequence based on pattern
  ...
  switch (pattern) {
    ...
    case "front_loaded":
      ...
      while (sequence.length < totalItems) {
        for (const topic of topics) {
          if (sequence.length < totalItems) sequence.push(topic);
        }
      }
      break;

    case "spaced":
      const weighted = topics.flatMap((t, idx) => Array(topics.length - idx).fill(t));
      for (let i = 0; i < totalItems; i++) {
...
// Create the MCP handler with the new API format
const handler = createMcpHandler(
  (server) => {
    // TOOL 1: CREATE STUDY PLAN
    server.tool(
      "create_study_plan",
      `Generate an interleaved study plan that mixes multiple subjects/topics for optimal learning.

Interleaving helps the brain differentiate between concepts, strengthening memory and improving knowledge transfer.

Available patterns:
- random: Unpredictable order for maximum discrimination
- systematic_short: ABCABC - Quick rotations
- systematic_extended: AABBCC - Longer focus periods
- front_loaded: Deep study then rapid review
- spaced: Integrated with spaced repetition`,
      {
        ...
        duration_minutes: z.number().min(15).max(480).describe("Total study duration in minutes"),
        pattern: z.enum(["random", "systematic_short", "systematic_extended", "front_loaded", "spaced"])
          .default("systematic_short").describe("Interleaving pattern to use"),
        breaks_enabled: z.boolean().default(true).describe("Include break recommendations")
      },
      async ({ user_id, subjects, duration_minutes, pattern, breaks_enabled }) => {
        ...
        const allTopics = subjects.flatMap(s => s.topics.map(t => `${s.name}: ${t}`));
        ...
        for (let i = 0; i < sequence.length; i++) {
          ...
          schedule.push({
            time: timeStr,
            topic: sequence[i],
            activity: i % 3 === 0 ? "Learn new concepts" : i % 3 === 1 ? "Practice problems" : "Review & self-test"
          });
    ...
    server.tool(
      ...
      `Create an interleaved quiz that mixes questions from multiple topics.

This helps learners practice discrimination - identifying which strategy applies to each problem.`,
      {
        ...
        quiz_length: z.number().min(5).max(50).default(10).describe("Number of questions"),
        shuffle_options: z.boolean().default(true).describe("Shuffle multiple choice options"),
        include_topic_hints: z.boolean().default(false).describe("Show topic for each question")
      },
      async ({ topics, quiz_length, shuffle_options, include_topic_hints }) => {
        ...
        const quiz = selectedQuestions.map((q, idx) => {
          const formattedQ: Record<string, unknown> = {
            number: idx + 1,
            question: q.question,
            difficulty: q.difficulty,
            type: q.options && q.options.length > 0 ? "multiple_choice" : "open_ended"
          };

          if (include_topic_hints) formattedQ.topic_hint = q.topic;
          ...
          content: [{
            ...
            text: JSON.stringify({
              ...
              quiz: {
                ...
                description: "Questions mixed from multiple topics to enhance discrimination learning",
                total_questions: quiz.length,
                topic_distribution: topicDistribution,
                questions: quiz,
                interleaving_benefit: "By mixing question types, you practice identifying WHICH strategy to use"
              },
    ...
    server.tool(
      ...
      `Create interleaved flashcard decks across multiple topics.
...
Flashcards are shuffled across topics to maximize the interleaving effect.`,
    ...
    server.tool(
      ...
      `Get a shuffled, interleaved set of flashcards for study.
...
Cards are shuffled across topics to create an interleaved practice session.`,
      ...
      async ({ user_id, topics, count, pattern }) => {
        const userData = learningData.get(user_id);
        ...
        switch (pattern) {
          case "balanced":
            ...
            while (selectedCards.length < count) {
              ...
              idx++;
              if (Array.from(topicGroups.values()).every(g => g.length === 0)) break;
            }
            break;

          case "systematic":
            selectedCards = [...allCards].sort((a, b) => a.topic.localeCompare(b.topic)).slice(0, count);
            break;
          ...
          content: [{
            ...
            text: JSON.stringify({
              ...
              session: {
                ...
                interleaving_tip: "Try to identify the topic before flipping each card",
    ...
    server.tool(
      "log_study_session",
      "Log a completed study session to track learning progress.",
      {
        user_id: z.string().describe("Unique identifier for the user"),
        ...
        quiz_score: z.number().min(0).max(100).optional().describe("Quiz score if taken"),
        notes: z.string().optional().describe("Optional notes about the session")
      },
      async ({ user_id, topics_studied, duration_minutes, quiz_score, notes }) => {
        if (!learningData.has(user_id)) {
          learningData.set(user_id, { subjects: new Map() });
        }
        ...
        for (const topic of topics_studied) {
          ...
          if (quiz_score !== undefined) {
            subjectData.quizHistory.push({ date: sessionDate, score: quiz_score, topic });
          }
        }
    ...
    server.tool(
      ...
      "Get learning progress and recommendations based on study history.",
      {
        user_id: z.string().describe("Unique identifier for the user"),
        include_recommendations: z.boolean().default(true).describe("Include personalized recommendations")
      },
      async ({ user_id, include_recommendations }) => {
        const userData = learningData.get(user_id);
        ...
        if (!userData || userData.subjects.size === 0) {
          return {
            content: [{
              ...
              text: JSON.stringify({
                success: false,
                error: "No learning data found. Start by creating a study plan or logging sessions."
              }, null, 2)
            }]
          };
        }
        ...
        for (const [topicName, subjectData] of userData.subjects) {
          ...
          topicProgress[topicName] = {
            ...
            last_studied: sessions.length > 0 ? sessions[sessions.length - 1].date : null
          };
        }
        ...
        if (include_recommendations) {
          ...
          if (topics.length >= 2) {
            recommendations.push("Continue interleaving topics - mixing subjects strengthens learning");
          }
    ...
    server.tool(
      ...
      "Get information about available interleaving patterns and when to use them.",
      {},
      async () => {
        return {
          content: [{
            ...
            text: JSON.stringify({
              ...
              patterns: {
                random: {
                  ...
                  best_for: "Advanced learners, exam preparation",
                  difficulty: "High"
                },
                systematic_short: {
                  ...
                  best_for: "Beginners to interleaving, multiple new topics",
                  difficulty: "Medium"
                },
                systematic_extended: {
                  ...
                  best_for: "Complex topics requiring deeper focus",
                ...
                front_loaded: {
                  ...
                  best_for: "New material introduction",
                  difficulty: "Low to Medium"
                },
                spaced: {
                  name: "Spaced Interleaving",
                  example: "Personalized based on topic strength",
                  description: "Spaced repetition integrated with interleaving",
                  best_for: "Long-term retention",
                  difficulty: "Variable"
                }
              },
              research_insights: [
                "Interleaving is more effective than blocking for problem-solving",
                "The 'desirable difficulty' of interleaving improves long-term retention",
                "Interleaving teaches WHICH strategy to use, not just HOW",
